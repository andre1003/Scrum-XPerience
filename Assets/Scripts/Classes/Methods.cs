...
namespace Methods {
    ...
    public class XP : Methododology {
        public XP() {
            ...
            this.functionDescription = new Dictionary<string, string>() {
                { "Software Manager", "O gerente de projetos é o responsável por gerenciar o desenvolvimento do software, garantindo que o sistema seja desenvolvido de acordo com o desejo do cliente. O aluno que escolher este papel deve se responsabilizar por gerenciar o desenvolvimento do projeto fictício, conduzindo escolhas de forma semelhante a uma junção de Product Owner com Scrum Master." },
                { "Test Engineer", "Sem dúvidas um papel crucial para a XP, o Engenheiro de Testes é o responsável pelos seguintes aspectos: definir quais testes serão aplicados no software a ser desenvolvido; auxiliar na elaboração desses testes (produzindo testes automatizados com as devidas ferramentas); e analisar os resultados dos testes aplicados na iteração desenvolvida, viabilizando ou não seu release. Assim, o aluno que escolher esse papel fica responsável pela parte mais importante do desenvolvimento segundo a metodologia XP: os testes. As escolhas dele certamente terão grande impacto na condução correta ou não dos valores da XP, o que impacta no desenvolvimento do software." },
                { "Developer", "O desenvolvedor é o responsável pela elaboração dos incrementos e, consequentemente, do software final a ser entregue. Desse modo, ele deve trabalhar em uma equipe auto organizável, de modo a realizarem uma seleção dos requisitos a serem implementados por incremento. O principal ponto é a entrega frequente de releases, construindo o software de “pouco em pouco”. Assim, o jogador que escolher esse papel terá basicamente as mesmas escolhas do membro do time de desenvolvimento Scrum, apenas adequando os nomes e algumas ações para a metodologia XP." }
            };
        }
    }
}
